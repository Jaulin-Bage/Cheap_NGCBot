        ...
        if accessToken:
            ...
            if task_id:
                ...
                if imgUrl:
                    ...
                    return savePath
                return None
    def getDoubaoAi(self, content, messages):
        op(f'[*]: 正在调用doubao对话接口... ...')
        try :
            client = Ark(base_url="https://ark.cn-beijing.volces.com/api/v3",)
            completion = client.chat.completions.create(
            model="ep-20250206202741-9lhg7",
            messages = [{"role": "system", "content": "你是AI对话助手，请模拟人的语气，在微信聊天中简要准确地回复信息。"},
    ...
    def getAi(self, content):
        ...
        for i in range(1, 9):
            ...
            if aiModule == 'localDeepSeek':
                ...
                think2 = result.find('</think>',0)
                think2 = result.find('</think>',think2+1)
                result = result[think2+8:]
            if aiModule == 'doubao':
                result, self.doubaoMessages = self.getDoubaoAi(content, self.DoubaoMessages)
            if aiModule == 'random':
                import random
